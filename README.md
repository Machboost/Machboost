...
[![CI](https://github.com/Machboost/Machboost/actions/workflows/ci.yml/badge.svg)](https://github.com/Machboost/Machboost/actions/workflows/ci.yml)
...
```sh
git clone https://github.com/Machboost/Machboost.git
cd machboost
python3 -m venv .venv
...
```sh
pip install "machboost[mlx] @ git+https://github.com/Machboost/Machboost.git"
pip install "machboost[vision] @ git+https://github.com/Machboost/Machboost.git"
```

Update an existing install:

```sh
pip install --upgrade "machboost[mlx] @ git+https://github.com/Machboost/Machboost.git"
```
...
| `vision_cache_qwen25_3b_20260714.json` | `mlx-community/Qwen2.5-VL-3B-Instruct-4bit` | repeated questions over one image | 12 | 100% | 17.96x |
...
The visual artifact compares 12 uncached requests with 12 accelerated requests on the same resident Qwen2.5-VL 3B model. Median wall time fell from 2.861 seconds uncached to 0.158 seconds on the accelerated path; median paired speedup was 17.96x and median TTFT speedup was 19.12x. All paired outputs and expected fixture answers matched. Eleven of 12 accelerated rows reused a 1,018-token visual prefix. The remaining row deliberately repeated the priming prompt, so it only reused projected image features and reached 1.51x. These are warm repeated-image results on one machine and model, not a claim about first-view latency, decode throughput, changed images, or arbitrary visual workloads.
