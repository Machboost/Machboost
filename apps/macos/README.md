...
- local SwiftData conversation history with search, rename, Markdown export,
  and delete
- tested-model catalog, local config/template capability detection, explicit
  downloads, architecture preflight, aggregate
...
```sh
./scripts/release_macos.sh 0.16.15 --local
open dist/macos/MachBoost-0.16.15-arm64.dmg
```
...
./scripts/release_macos.sh 0.16.15
./scripts/publish_macos_release.sh 0.16.15 ./release-notes/0.16.15.md
```
...
signed Sparkle appcast. The publisher requires an existing `v0.16.15` tag, an
