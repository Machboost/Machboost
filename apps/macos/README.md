...
```sh
./scripts/release_macos.sh 0.13.0-local --local
open dist/macos/MachBoost-0.13.0-local-arm64.dmg
```
...
Public DMG releases need MachBoost Maintainers's Developer ID, Apple team and
...
./scripts/release_macos.sh 0.13.0
./scripts/publish_macos_release.sh 0.13.0 ./release-notes/0.13.0.md
```
...
signed Sparkle appcast. The publisher requires an existing `v0.13.0` tag, an
