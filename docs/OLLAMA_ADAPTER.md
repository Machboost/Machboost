...
The local checkout at `/workspace/Downloads/ollama-main` is not a git repository, so the first pass should be carried as a documented patch or a proper fork rather than mutating the downloaded folder in place.
