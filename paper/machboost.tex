...
\begin{abstract}
Autoregressive decoding remains serial even when a model fits comfortably on a laptop. MachBoost tests a narrow way around that constraint: use tokens from explicit local context as speculative candidates, verify a block with the target model, and return to the runtime's native decoder when no candidate exists. The current implementation combines context-only n-gram lookup, block verification, in-place MLX KV-cache rewind, verifier continuation tokens, native-compatible prompt prefill, and \texttt{mlx-lm} streaming. We compare it with native cached generation rather than a synchronous Python token loop. On an Apple M1 Max, using \path{mlx-community/Qwen2.5-3B-Instruct-4bit}, the default profile reaches a 1.96$\times$ median paired speedup on a 64-token code-continuation fixture over five fresh-nonce repeats. An experimental one-token re-entry profile reaches 1.58$\times$ on copied retrieval answers. All 30 rows across the two profiles match their paired native greedy outputs; open-ended controls stay near the native path. The results fall short of a universal 2$\times$ accelerator. They identify a conditional operating region, show how re-entry broadens coverage, and expose why earlier measurements against stateless or per-token baselines overstated the gain.
\end{abstract}
...
Measurements were collected on an Apple M1 Max with 10 CPU cores and 32 GB unified memory, running macOS 26.5.2. The environment used Python 3.13.3, MLX 0.31.2, \texttt{mlx-lm} 0.31.3, and MachBoost 0.1.4. No thermal or performance warning was reported immediately before or after the run. The model was \path{mlx-community/Qwen2.5-3B-Instruct-4bit}.
