...
\section{Conclusion}
MachBoost is a practical experiment in using nearby text as the draft source for speculative decoding. It is not a shortcut for every prompt. On the measured MLX suite, however, it matches serial greedy decoding across all 35 initial runs and reaches a 1.38$\times$ median speedup overall, with 1.60--1.62$\times$ speedups on the strongest grounded tasks. A stricter 64-token run adds 30 exact-match rows and a 3.03$\times$ median speedup against stateless greedy decoding, including real README, code, and paper-source fixtures. The package design reflects the main lesson from the results: use the drafter when local context predicts the next tokens, and fall back when it does not.
